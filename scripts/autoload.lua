...
function normalize_autoload_playlist_order(files)
    ...
    local entries = {}
    local wraps = 0
    local previous_order = nil
    for index, item in ipairs(playlist) do
        local filename = item.filename
        if not filename or not order[filename] or seen[filename] then
            return
        end
        seen[filename] = true
        if previous_order and order[filename] < previous_order then
            wraps = wraps + 1
        end
        previous_order = order[filename]
        entries[index] = { filename = filename, order = order[filename] }
    end
    if wraps > 1 then
        return
    end
    ...
    for index, entry in ipairs(entries) do
        if playlist[index].filename ~= entry.filename then
    ...
    local current = {}
    for index, item in ipairs(playlist) do
        current[index] = item.filename
    end

    for target_index, target in ipairs(entries) do
        local current_index = nil
        for index, filename in ipairs(current) do
        ...
        if current_index and current_index ~= target_index then
            mp.commandv("playlist-move", current_index - 1, target_index - 1)
            table.remove(current, current_index)
            table.insert(current, target_index, target.filename)
        end
    end
end
...
function find_and_add_entries()
    ...
    if pl_count > 1 and autoloaded == nil then
        local ext_l = this_ext and string.lower(this_ext) or ""
        local extensions = EXTENSIONS
        if o.same_type then
            if EXTENSIONS_VIDEO[ext_l] ~= nil then
                extensions = EXTENSIONS_VIDEO
            elseif EXTENSIONS_AUDIO[ext_l] ~= nil then
                extensions = EXTENSIONS_AUDIO
            elseif EXTENSIONS_IMAGES[ext_l] ~= nil then
        ...
        scan_dir(dir, path, dir_mode, separator, 0, files, extensions)
        normalize_autoload_playlist_order(files)
        msg.debug("stopping: manually made playlist")
        return
    elseif (pl_count == 1 and (not this_ext or EXTENSIONS[string.lower(this_ext)] == nil)) then
        msg.debug("stopping: manually made playlist")
        return
    else
        if pl_count == 1 then
